---
name: rote
description: Use rote-toolkit to create, update, delete, search, and list Rote notes and related resources through the Rote OpenKey API. Trigger this skill when Codex needs to operate a Rote instance from terminal workflows or AI-agent flows, especially for note CRUD, article creation, reactions, profile updates, permission checks, or MCP server setup via the rote-toolkit CLI, SDK, or MCP server.
---
...
- Check `~/.rote-toolkit/config.json` before operating. If missing, run `rote config`.
